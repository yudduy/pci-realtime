...
- `src/pci_realtime/agent_intake.py` + `agent_research.py` — collective evidence intake: agents submit official-source URLs (`agent_research.py` gates on `OFFICIAL_SOURCE_DOMAINS` + provision match; `agent_intake.py` canonicalizes, scores, and folds into the index). Wired through `service.py` and the `scripts/run_agent_research_intake.py` / `provision_agent_evidence.py` entry points, not the two main pipelines.
- `apps/web` — Next 16 / React 19, App Router. `lib/data.ts` fetches public views with publishable keys only; `lib/market-model.ts` merges rows into UI `PolicyMarket` objects. Routes: `/`, `/about`, `/connect`, `/policies/[code]`, `/provisions/[code]`, plus hosted `/mcp`, `/llms.txt`. `/dashboard` is an edge-level 301 to `/` (`next.config.mjs`); `/conncet` catches the common typo and redirects to `/connect`.
